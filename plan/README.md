...
- Record explicit user scores as authoritative when provided. When no explicit score has been provided yet, continue the work and maintain a conservative provisional score based on the current evidence instead of waiting or using a fixed hardcoded default number, but do not set that provisional score above `50`.
